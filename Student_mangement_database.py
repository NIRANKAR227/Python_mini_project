...
def run_query(query,parameters=()):
    conn=psycopg2.connect(dbname="studentdb",user=os.getenv("DB_USER"),password=os.getenv("DB_PASS"),host="localhost", port="5432")
    curr=conn.cursor()
    query_result=None
    try:
...
age_entry.grid(row=2,column=1,pady=2,sticky="w")
